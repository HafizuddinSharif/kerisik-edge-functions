-- Migration: Tokenize ingredient-aware browsable recipe search.
  ...
  WHERE (
    ...
    AND (
      ...
      OR NOT EXISTS (
        SELECT 1
        FROM unnest(
          regexp_split_to_array(lower(btrim(p_search)), '[,[:space:]]+')
        ) AS token(value)
        WHERE token.value <> ''
          AND NOT (
            lower(br.meal_name) LIKE '%' || token.value || '%'
            OR lower(coalesce(br.meal_description, '')) LIKE '%' || token.value || '%'
            OR lower(array_to_string(br.tags, ' ')) LIKE '%' || token.value || '%'
            OR EXISTS (
              SELECT 1
...
IS 'List published browsable recipes with optional platform/tag filters and all-token text search across recipe text, tags, and denormalized ingredient names. p_include_dev_only should be true only in development.';
